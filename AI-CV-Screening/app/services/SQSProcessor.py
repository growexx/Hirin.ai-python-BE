    ...
    async def process_queue_message_in_batches(self):
        try:
            ...
            for _ in range(self.num_workers):
                worker_task = asyncio.create_task(resumeProcess.getCVScore(task_queue,self.queueUrl))
                workers.append(worker_task)
    
            async with aioboto3.Session().client('sqs',region_name=self.AWS_REGION) as sqs:
