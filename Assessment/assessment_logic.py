...
def unified_assessment(metadata, role, job_description, level_of_seniority, questions_json, api_key, model):
    try:
        ...
        logging.info("Unified assessment completed successfully.")
...
def add_skills_to_assessment_result(assessment_result, assessments):
    try:
        ...
        logging.info("Skills successfully added to assessment result.")
...
def calculate_candidate_score(questions):
    try:
        ...
        logging.info(f"Candidate score calculated: {score}")
        return score
    except Exception as e:
        logging.error(f"Failed to calculate candidate score: {e}")
        raise

def calculate_total_possible_score(questions):
    try:
        total_score = len(questions) * 5
        logging.info(f"Total possible score calculated: {total_score}")
        return total_score
    except Exception as e:
        logging.error(f"Failed to calculate total possible score: {e}")
        raise
...
def skill_wise_assessment(questions, skill_scores, api_key, model):
    try:
        ...
        logging.error(f"Failed to complete skill-wise assessment: {e}")
        raise
