...
#     def count_and_track_people(self, fps=30, distance_threshold=0.9):
#         """Detects and tracks people across frames."""
#         try:
#             logger.info("Starting people detection and tracking.")
#             embedder = FaceNet()
#             unknown_mapping = {}
...
#         tab_switch_count = api_data['tab_switch_count']
#         tab_switch_time = api_data['tab_switch_time']
#         multiple_faces = api_data['result'].get('multipleFacesDetected', False)
#         time_not_in_frame_str = api_data['result']['timePersonWasNotInFrame']
...
#             self.count_and_track_people()
#             formatted_output = self.format_output()
#             logger.info(f"Video processing completed for metadata: {metadata}")
#             return formatted_output
#         except Exception as e:
#             logger.error(f"Error processing video for metadata {metadata}: {e}")
#             raise
#         finally:
#             self.cleanup()
